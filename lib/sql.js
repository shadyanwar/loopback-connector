...
SQLConnector.prototype.buildOrderBy = function(model, order) {
  ...
  for (var i = 0, n = order.length; i < n; i++) {
    if (typeof order[i] === 'string') {
      ...
      let orderValue = Object.values(order[i])[0];
      var vars = orderValue.match(/(?<=\$:).*?(?=:\$)/gs);
      for (var j = 0, m = vars.length; j < m; j++) {
        var modelVar = self.columnEscaped(model, vars[j]);
        orderValue = orderValue.replace(/\$:.*?:\$/, modelVar);
      }
      clauses.push(orderValue);
    }
  }
